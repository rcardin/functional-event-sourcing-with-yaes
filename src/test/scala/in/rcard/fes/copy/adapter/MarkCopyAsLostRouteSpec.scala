...
class MarkCopyAsLostRouteSpec extends AnyFlatSpec with SyncSpec with Matchers {

  private val mockedMarkCopyAsLostUseCase = new MarkCopyAsLostUseCase {
  ...
  private val underTest: Sync ?=> YaesRoutes = YaesRoutes(
    MarkCopyAsLostRoute(mockedMarkCopyAsLostUseCase).markCopyAsLostRoute
  )

  "MarkCopyAsLostRoute" should "return 200 if the copy is marked as lost successfully" in withSync {
    val request = Request(POST, s"/copies/$COPY_ID_VALUE/lost", Map.empty, "", Map.empty)
  ...
  it should "return 404 if the copy was never registered" in withSync {
    val request =
      Request(POST, s"/copies/$NOT_REGISTERED_COPY_ID_VALUE/lost", Map.empty, "", Map.empty)
  ...
  it should "return 409 if the copy is already lost" in withSync {
    val request =
      Request(POST, s"/copies/$ALREADY_LOST_COPY_ID_VALUE/lost", Map.empty, "", Map.empty)
  ...
  it should "return 500 if the use case raises an unexpected error" in withSync {
    val request =
      Request(POST, s"/copies/$UNEXPECTED_COPY_ID_VALUE/lost", Map.empty, "", Map.empty)
