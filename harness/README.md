...
behaviour (eight `checkc ... loop.out` assertions), so the `log()` wording on those paths is
asserted behaviour in both implementations; `harness/scala/test/LogParitySpec.scala` pins the same
needles in-memory.
